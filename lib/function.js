...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./ES_TEAMS-MEDIA/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./ES_TEAMS-MEDIA/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./ES_TEAMS-MEDIA/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./ES_TEAMS-MEDIA/trash/${filename}.mp4`)
    Promise.all([unlink(`./ES_TEAMS-MEDIA/trash/${filename}.mp4`), unlink(`./ES_TEAMS-MEDIA/trash/${filename}.gif`)])
    return buffer5
}
