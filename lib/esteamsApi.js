...
const MAX_VIDEO_BYTES = 50 * 1024 * 1024; // WhatsApp rejects large video uploads anyway; catch it early with a clear message instead of handing Baileys a huge file to buffer and thumbnail, which is heavy enough to crash a small host outright.
...
const downloadVideoSong = async (query) => {
	...
	try {
		...
		// HEAD failing for any other reason (server doesn't support it, network hiccup) --
		// don't block the send over that, just proceed without the size check.
	}
